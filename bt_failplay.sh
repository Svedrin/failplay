...
readonly FAILPLAY="$SCRIPT_DIR/failplay.py"
...
# Runs in the background while failplay is in the foreground. Kills
# failplay as soon as the bluetooth device disappears.
start_watchdog() {
    (
        while kill -0 "$failplay_pid" 2>/dev/null; do
            if ! is_bt_connected; then
                kill -TERM "$failplay_pid" 2>/dev/null
                exit 0
            fi
            sleep "$POLL_INTERVAL"
        done
...
cleanup() {
    ...
    stop_watchdog
    [[ -n "$failplay_pid" ]] && kill -TERM "$failplay_pid" 2>/dev/null
    exit 0
}
